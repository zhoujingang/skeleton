function drawSkeleton() {
    class DrawDom {
            ...
            for (let i = 0; i < nodes.length; i++) {
                ...
                if (
                    ...
                    if (w > 0 && h > 0 && l < this.win_w && t < this.win_h) {
                        ...
                        if (w == h && this.htmlFontSize > 0) { // 如果元素是圆形需要宽高转 rem
                            dom.width = this.pxToRem(w - pL - pR) + 'rem';
                            dom.height = this.pxToRem(h - pL - pR) + 'rem';
                        }
                        this.drawDom(dom)
         ...
         * @param {*} param0 
         */
        drawReactDom({ width, height, top, left, radius, subClas } = {}) {
            const styles = ['height:' + height];
            if (!subClas) {
                styles.push('top:' + top, 'left:' + left, 'width:' + width);
            }
         ...
         */
        getHtmlFontSize() {
            const fontSizePX = this.getStyle(document.documentElement, 'font-size');
            this.htmlFontSize = parseFloat(fontSizePX);
        }
        ...
        pxToRem(x) {
            return parseFloat(x / this.htmlFontSize).toFixed(3);
        }
        wPercent(x) {
